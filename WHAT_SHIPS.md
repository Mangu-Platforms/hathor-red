...
- **Mute control (dose-1.78)**: `toggleMute` in PlayerContext (same restore path as keyboard M); MusicPlayer volume row has a mute/unmute button with muted icon when volume is 0
...
Dose 1.78 mute button closed. Next: remaining Dose 2 (account) polish or Dose 3 home/playlists genre filter honesty (client SongList select vs server Home filter). No Dose 6+.
