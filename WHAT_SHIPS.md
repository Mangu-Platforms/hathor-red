...
Last updated: 2026-09-10 (dose-1.15 document PlayerContext placeholder on main).

## Ships today

> **CRITICAL (dose-1.15):** `client/src/contexts/PlayerContext.js` on `main` is a 24–35 byte placeholder string (`SEE_LOCAL_FILE_TOO_LARGE_FOR_INLINE` / prior push truncation). Live player is broken until a full restore lands. Historical full blobs exist under commits such as `afba348b` / `c971941b` / `45633ece`. Subsequent doses claimed on this file did not persist binary content through the agent push path.
...
**Restore `client/src/contexts/PlayerContext.js`** from a known-good historical blob (queue dedupe, insertNext move, shuffle-aware makeNext/insertNext, no loudness/waveform stubs). Agent push path truncates large files — use a transport that preserves full content. Then Settings platform-status polish / residual empty-state honesty.
