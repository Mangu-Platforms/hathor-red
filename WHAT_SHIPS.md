...
Last updated: 2026-09-11 (dose-1.28 streamAuth songId path match).
...
|------|--------|
| 0 Truth | Done |
| 1 Playback | Core done; streamAuth hardened (dose-1.23); stream token username (dose-1.24); authMiddleware normalize (dose-1.25); stream token positive-int ids (dose-1.26); authMiddleware positive-int userId (dose-1.27); streamAuth path songId match (dose-1.28) |
