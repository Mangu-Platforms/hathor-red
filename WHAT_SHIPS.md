...
- **Keyboard space/seek/volume (dose-1.119)**: Space toggles play/pause; ArrowLeft/Right seek ±5s; ArrowUp/Down volume ±0.05 (same input guards as N/P; uses existing KEYBOARD_* constants)
- **Keyboard mute (dose-1.120)**: M toggles mute via existing toggleMute (restores pre-mute volume); same input guards as N/P/space/arrows
...
Dose 1 keyboard mute (M) closed. Still no Dose 6+ until product prioritizes (e.g. multi-track queue persist, HLS player, OAuth).
