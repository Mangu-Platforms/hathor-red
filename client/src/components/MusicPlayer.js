...
const MusicPlayer = () => {
  ...
  const remainingSeconds = (() => {
    ...
    if (isShuffled && Array.isArray(shuffleOrder) && shuffleOrder.length === queue.length) {
      const pos = Number.isInteger(shufflePos) ? Math.max(0, shufflePos) : 0;
      ...
      <div className="player-main">
        ...
        <div className="player-extras">
          ...
          <div className="player-volume">
            ...
            <input type="range" min="0" max="1" step="0.01" value={volumeSafe} onChange={onVolumeChange} onKeyDown={onVolumeKeyDown} aria-label="Volume" aria-valuemin={0} aria-valuemax={100} aria-valuenow={Math.round(volumeSafe * 100)} aria-valuetext={volumeValueText} title={`Volume ${Math.round(volumeSafe * 100)}%`} />
          </div>
              ...
              {queue.map((song, idx) => (
                ...
                >
                  ...
                  <button
                    ...
                    aria-label={idx === queueIndex ? (isPlaying ? `Now playing: ${song.title}` : `Current track (paused): ${song.title}`) : `Play ${song.title}`}
                  >
