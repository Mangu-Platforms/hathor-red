...
const MusicPlayer = () => {
  const {
    ...
    queue, queueIndex, shuffleOrder, shufflePos, playAtIndex, removeFromQueue, moveInQueue, clearQueue,
  } = usePlayer();
  ...
  const onSpeedKeyDown = (e) => {
    const cur = Number.isFinite(playbackSpeed) ? playbackSpeed : 1;
  ...
  const remainingSeconds = (() => {
    ...
    // When shuffled, walk Fisher–Yates order from shufflePos; else linear from queueIndex.
      ...
      <div className="player-main">
        ...
        <div className="player-transport">
          ...
          <div className="player-progress-row">
            ...
            <div
              ...
              aria-valuetext={canSeek ? `${formatTime(progress)} of ${formatTime(duration)}` : 'No duration'}
            ...
            >
              <div className="player-progress-fill" style={{ width: canSeek ? `${Math.max(0, Math.min(100, (progress / duration) * 100))}%` : '0%' }} />
            </div>
        ...
        <div className="player-side">
          ...
          <input
            ...
            aria-valuemin={0}
            aria-valuemax={100}
            aria-valuenow={Math.round((Number.isFinite(volume) ? volume : 1) * 100)}
            aria-valuetext={`${Math.round((Number.isFinite(volume) ? volume : 1) * 100)} percent`}
          />
        ...
        <div id="player-queue-panel" className="player-queue-panel" role="listbox" aria-label="Play queue">
          <div className="player-queue-header">
            <span className="player-queue-header-title">
              ...
              {remainingLabel && (<span className="player-queue-remaining" title="Time left from current playhead">{' '}· {remainingLabel} left</span>)}
            </span>
            {queue.length > 0 && (
                ...
                return (
                  ...
                  >
                    ...
                    <div className="player-queue-row-actions">
                      <button type="button" className="player-queue-move" disabled={idx === 0} aria-label="Move up" onClick={(e) => { e.stopPropagation(); moveInQueue(idx, idx - 1); }}>↑</button>
                      <button type="button" className="player-queue-move" disabled={idx === queue.length - 1} aria-label="Move down" onClick={(e) => { e.stopPropagation(); moveInQueue(idx, idx + 1); }}>↓</button>
                      <button type="button" className="player-queue-remove player-queue-play-next" title="Play next" aria-label={`Play ${song.title} next`} onClick={(e) => { e.stopPropagation(); const target = Math.min(queueIndex + 1, queue.length - 1); if (idx !== target) moveInQueue(idx, target); }}>⤵</button>
