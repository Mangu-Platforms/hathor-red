      ...
      <div className="player-main">
        ...
        <div className="player-extras">
          ...
          <button
            ...
            title="Queue"
            aria-label={showQueue ? 'Hide queue' : (queue.length > 0 ? `Show queue, ${queue.length} tracks` : 'Show queue')}
          ...
          <button
            ...
            onClick={() => { setShowAdvanced(!showAdvanced); setShowQueue(false); }}
        ...
        <div id="player-queue-panel" className="player-queue-panel" role="listbox" aria-label="Play queue">
          <div className="player-queue-header">
            <span className="player-queue-header-title">
              ...
              {remainingLabel && (
                <span className="player-queue-remaining" title="Remaining from current position through end of queue">
                  {' '}· {remainingLabel} left
                </span>
              )}
              ...
              {queue.map((song, idx) => (
                ...
                >
                  ...
                  {idx !== queueIndex && (
                    <button
                      type="button"
                      className="player-queue-remove player-queue-play-next"
                      title="Play next"
                      aria-label={`Play ${song.title} next`}
                      onClick={(e) => {
                        e.stopPropagation();
                        // Move this row to immediately after current index
                        const target = Math.min(queueIndex + 1, queue.length - 1);
                        if (idx !== target) moveInQueue(idx, target);
                      }}
                    >
                      <svg width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={1.5} aria-hidden="true">
                        <path strokeLinecap="round" strokeLinejoin="round" d="M3 5v14l8-7-8-7zm9 0h2v14h-2V5zm4 0h2v14h-2V5z" />
                      </svg>
                    </button>
                  )}
                  <button
