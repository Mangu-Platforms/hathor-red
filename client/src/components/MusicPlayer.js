            ...
            <ul className="player-queue-list">
              {queue.map((song, idx) => (
                ...
                >
                  ...
                  <button type="button" className={`player-queue-item ${idx === queueIndex ? 'active' : ''}`} onClick={() => playAtIndex(idx)}>
                    ...
                    {idx === queueIndex && (<span className="player-queue-now" aria-label={isPlaying ? 'Now playing' : 'Current track (paused)'} title={isPlaying ? 'Now playing' : 'Current track (paused)'}>{isPlaying ? '▶' : '❚❚'}</span>)}
                  </button>
                  {idx !== queueIndex && (
