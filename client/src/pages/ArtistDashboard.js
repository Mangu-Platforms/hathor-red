  ...
  return (
    <div className="oly-page">
      ...
      <div className="oly-section">
        <h2>Top tracks</h2>
        {intelOff === true ? (
          <div className="oly-empty">Intel analytics disabled on this server (FEATURE_INTEL off or routes missing).</div>
        ) : topTracks.length === 0 ? (
          <div className="oly-empty">
            No plays recorded yet for your uploads.
            {mySongs.length === 0
              ? ' Upload a track above, then play it from Home or Library so listening history can populate this table.'
              : ' Play your tracks from Home or share a room so listeners generate history for the last 30 days.'}
          </div>
        ) : (
      ...
      <div className="oly-section">
        ...
        {intelOff === true ? (
          ...
          <div className="oly-empty">
            No geography data yet — plays without a country still count in overview, but this table fills after listeners with geo signals appear.
            {mySongs.length === 0 && ' Upload and share tracks to start collecting listener location.'}
          </div>
        ) : (
          <table className="oly-table">
      ...
      <div className="oly-section">
        ...
        {commerceOff === true ? (
          <div className="oly-empty">Commerce is not available on this server (FEATURE_COMMERCE off or routes missing).</div>
        ) : revenueByTrack.length === 0 ? (
          <div className="oly-empty">
            No sales yet — list a track in the Store when commerce is live.
            {features?.workerLive === false && ' Background worker is not running; commerce jobs may stall until it is up.'}
          </div>
        ) : (
          <table className="oly-table">
    ...
    <div className="oly-section">
      ...
      {tracks.length === 0 ? (
        ...
        <table className="oly-table">
          ...
          <tbody>
            {tracks.map((t) => {
              const p = pipelineBySong[t.songId] || {};
              return (
                <tr key={t.songId}>
                  <td>{t.title}</td>
                  <td>
                    {p.loading ? (
                      ...
                      <span className="oly-msg err" style={{ display: 'inline' }}>{p.error}</span>
                    ) : p.data ? (
                      <span className="muted">
                        {(p.data.status || p.data.pipelineStatus || 'ok')}
                        {p.data.hasWaveform ? ' · waveform' : ''}
                        {p.data.variants?.length ? ` · ${p.data.variants.length} variants` : ''}
                      </span>
                    ) : (
                  ...
                  <td>
                    {p.jobId != null ? (
                      <span className="muted">
                        #{p.jobId}
                        {p.jobStatus ? ` · ${p.jobStatus}` : ''}
                        {p.jobAttempts != null && p.jobMaxAttempts != null
                          ? ` · ${p.jobAttempts}/${p.jobMaxAttempts}`
                          : ''}
                        {p.jobError ? ` · ${p.jobError}` : ''}
                        {p.jobWorkerLive === false ? ' · worker not live' : ''}
                      </span>
                    ) : (
                      <span className="muted">—</span>
                    )}
                  ...
                  <td style={{ whiteSpace: 'nowrap' }}>
                    ...
                    <button
                      ...
                      disabled={p.reprocessBusy}
                    >
                      {p.reprocessBusy ? 'Queuing…' : 'Reprocess'}
                    </button>
                    {p.jobId != null && p.jobStatus && !TERMINAL_JOB.has(p.jobStatus) && (
                      <button
                        type="button"
                        className="oly-btn-ghost"
                        onClick={() => startJobPoll(t.songId, p.jobId)}
                      >
                        Poll
                      </button>
                    )}
...
function UploadSection({
  ...
  const workerNote = (() => {
    if (!features) return null;
    if (features.worker === false) {
      return 'FEATURE_WORKER is off — uploads still save, but transcode/embed jobs will not run.';
    }
    if (features.workerLive === false) {
      return 'Background worker is not live — queued jobs may stall until it starts.';
    }
    return null;
  })();
