...
export const PlayerProvider = ({ children }) => {
  ...
  // dose-1.74: record a completed listen into listening_history (server /record-listening).
  // Only fires on natural track end; skips under 5s to avoid noise from skip-to-end.
  ...
  // dose-1.71 + dose-1.73: keyboard media controls
  useEffect(() => {
    if (!isAuthenticated) return undefined;
    const onKeyDown = (e) => {
      const t = e.target;
      ...
      switch (e.key) {
        ...
        case 'ArrowLeft': {
          ...
          const next = Math.max(0, t0 - KEYBOARD_SEEK_SEC);
        ...
        case 'ArrowRight': {
          ...
          const next =
            Number.isFinite(d) && d > 0
              ? Math.min(d, t0 + KEYBOARD_SEEK_SEC)
              : t0 + KEYBOARD_SEEK_SEC;
          safeSetCurrentTime(audio, next);
          setProgress(next);
          persistPlayback();
          break;
  ...
  useEffect(() => {
    ...
    const handlers = {
      ...
      seekto: (details) => {
        ...
        const d = audio.duration;
        const clamped =
          Number.isFinite(d) && d > 0 ? Math.max(0, Math.min(d, t)) : Math.max(0, t);
        safeSetCurrentTime(audio, clamped);
        setProgress(clamped);
        persistPlayback();
      },
