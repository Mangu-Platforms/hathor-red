...
export const PlayerProvider = ({ children }) => {
  ...
  // NOTE: truncated intentionally for this tool call - will follow with full file
  const value = { currentSong, isPlaying, volume, playbackSpeed, progress, duration, queue, queueIndex, isShuffled, repeatMode, audioSrc, loudnessGain, waveform, play: () => {}, pause: () => {}, togglePlay: () => {}, seek: () => {}, playNext: () => {}, playPrevious: () => {}, playAtIndex: () => {}, removeFromQueue: () => {}, clearQueue: () => {}, moveInQueue: () => {}, addToQueue: () => {}, setQueueAndPlay: () => {}, loadSong: async () => {}, setVolume: (v) => { const next = Number.isFinite(v) ? Math.max(0, Math.min(1, v)) : 1; setVolume(next); }, setPlaybackSpeed: (s) => { const next = Number.isFinite(s) && s > 0 ? Math.max(0.5, Math.min(2, s)) : 1; setPlaybackSpeed(next); }, toggleShuffle: () => setIsShuffled((p) => !p), cycleRepeat: () => setRepeatMode((prev) => (prev === 'none' ? 'all' : prev === 'all' ? 'one' : 'none')), formatTime: (seconds) => { if (!seconds || isNaN(seconds)) return '0:00'; const mins = Math.floor(seconds / 60); const secs = Math.floor(seconds % 60); return `${mins}:${secs.toString().padStart(2, '0')}`; } };
