...
export const PlayerProvider = ({ children }) => {
  ...
  }, [playNext, playPrevious, play, pause, seek, audio]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!state || (state.current_song_id == null && state.currentSongId == null)) {
        ...
        let song = null;
        try {
          song = await musicService.getSong(songId);
        } catch (e) {
          console.warn('hydrate: song fetch failed', e);
          hydratedRef.current = true;
          return;
        }
        if (cancelled || !song) {
        ...
        const pos = Number(state.position ?? state.playback_position ?? 0);
        const vol = Number(state.volume);
        const speed = Number(state.playbackSpeed ?? state.playback_speed);
        ...
        await loadSong(song, { autoplay: false, startAt: Number.isFinite(pos) ? pos : 0 });
        hydratedRef.current = true;
      } catch (err) {
        console.warn('hydrate playback failed', err);
        hydratedRef.current = true;
      }
    })();
  ...
  useEffect(() => {
    ...
    persistTimer.current = setTimeout(() => {
      ...
      musicService.updatePlaybackState(payload).catch((err) => {
        console.warn('persist playback state failed', err);
      });
    }, PERSIST_DEBOUNCE_MS);
    return () => {
      if (persistTimer.current) clearTimeout(persistTimer.current);
