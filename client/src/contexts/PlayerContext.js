...
export const PlayerProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const safeLoudnessGain = Number.isFinite(loudnessGain) ? loudnessGain : 1;
    audio.volume = Math.max(0, Math.min(1, safeVolume * safeLoudnessGain));
  }, [volume, loudnessGain, audio]);
  useEffect(() => {
    const safePlaybackSpeed = Number.isFinite(playbackSpeed) ? playbackSpeed : 1;
    audio.playbackRate = Math.max(0.5, Math.min(2, safePlaybackSpeed));
  }, [playbackSpeed, audio]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Dose-1.51: register hydrate resume seek via durationMetaCleanupRef so
        // concurrent loadSong / stream recovery can drop a stale listener.
        const onMeta = () => {
          audio.removeEventListener('loadedmetadata', onMeta);
          if (durationMetaCleanupRef.current === cleanup) durationMetaCleanupRef.current = null;
  ...
  const value = {
    ...
    setVolume: (v) => {
      const nextVolume = Number.isFinite(v) ? Math.max(0, Math.min(1, v)) : 1;
      setVolume(nextVolume);
      persistPlaybackState({ volume: nextVolume });
    },
    setPlaybackSpeed: (s) => {
      const nextPlaybackSpeed = Number.isFinite(s) ? Math.max(0.5, Math.min(2, s)) : 1;
      setPlaybackSpeed(nextPlaybackSpeed);
      persistPlaybackState({ playbackSpeed: nextPlaybackSpeed });
    },
    toggleShuffle,
