...
export const PlayerProvider = ({ children }) => {
  ...
  const insertNext = useCallback((song) => {
    if (!song || song.id == null) return false;
    const q = queueRef.current;
    const existingIdx = q.findIndex((s) => s && s.id === song.id);
    if (existingIdx >= 0) {
      // Already in queue: move row to play-next slot (honest "Play next").
      // Skip when already current or already next.
      if (existingIdx === queueIndexRef.current) return true;
      const target = Math.min(queueIndexRef.current + 1, q.length - 1);
      if (existingIdx === target) return true;
      if (isShuffledRef.current && shuffleOrderRef.current.length === q.length) {
        // Under shuffle: reorder shuffleOrder so chosen track is next after current shuffle pos.
        setShuffleOrder((so) => {
          const curPos = Number.isInteger(shufflePosRef.current) ? shufflePosRef.current : 0;
          const fromPos = so.indexOf(existingIdx);
          if (fromPos < 0) return so;
          const next = so.filter((_, i) => i !== fromPos);
          const insertAt = Math.min(curPos + 1, next.length);
          next.splice(insertAt, 0, existingIdx);
          return next;
        });
        return true;
      }
      // Linear: move queue row.
      const from = existingIdx;
      const to = target;
      setQueue((prev) => {
        const next = [...prev];
    ...
    setQueue((prev) => {
      ...
      if (isShuffledRef.current) {
        setShuffleOrder((so) => {
          ...
          // Insert new index at after current shuffle position.
          const curPos = Number.isInteger(shufflePosRef.current) ? shufflePosRef.current : 0;
          const mapped = so.map((i) => (i >= at ? i + 1 : i));
          const insertAt = Math.min(curPos + 1, mapped.length);
          mapped.splice(insertAt, 0, at);
          return mapped;
        });
      }
      return next;
  ...
  const makeNext = useCallback((idx) => {
    ...
    if (isShuffledRef.current && shuffleOrderRef.current.length === q.length) {
      ...
      setShuffleOrder((so) => {
        const curPos = Number.isInteger(shufflePosRef.current) ? shufflePosRef.current : 0;
        const fromPos = so.indexOf(idx);
        if (fromPos < 0 || fromPos === curPos + 1) return so;
        const next = so.filter((_, i) => i !== fromPos);
        const insertAt = Math.min(curPos + 1, next.length);
  ...
  useEffect(() => {
    ...
    try {
      ...
      navigator.mediaSession.setActionHandler('seekforward', () => {
        seek((Number.isFinite(audio.currentTime) ? audio.currentTime : 0) + MEDIA_SESSION_SEEK_SEC);
      });
    } catch (_) {}
    return () => {
      try {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const spd = Number(state.playback_speed ?? state.playbackSpeed);
        if (Number.isFinite(spd) && spd > 0) setPlaybackSpeedState(Math.max(0.5, Math.min(2, spd)));
  ...
  const value = {
    ...
    shuffleOrder, shufflePos, isShuffled, repeatMode, audioSrc,
