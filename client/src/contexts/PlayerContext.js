...
export const PlayerProvider = ({ children }) => {
  ...
  /**
   * Resolve the next queue index.
   * When wrap is false (repeat-none + manual Next), return null at end of order
   * so playback stops — same as natural end — instead of looping forever.
   * When wrap is true (repeat-all, or natural-end path that already gated), use modulo.
   */
  ...
  const playNext = useCallback(async () => {
    ...
    // Dose 1.31: under repeat-none, manual Next at the last track stops (does not wrap).
    // repeat-all / repeat-one still advance or loop via handleEnded / wrap=true.
    const wrap = repeatMode !== 'none';
    const next = resolveNextIndex(wrap);
    if (next == null) {
      // End of order with repeat-none — pause and persist like natural end.
      setIsPlaying(false);
      audio.pause();
  ...
  }, [queue, resolveNextIndex, isShuffled, shuffleOrder, loadSong, audio, currentSong, preloadNext, persistPlaybackState, repeatMode]);
