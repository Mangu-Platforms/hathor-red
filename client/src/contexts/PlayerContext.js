...
export const PlayerProvider = ({ children }) => {
  ...
  useEffect(() => {
    const onKeyDown = (e) => {
      ...
      if (key === ' ' || key === 'k' || key === 'K') {
        ...
        e.preventDefault();
        setVolume(Math.min(1, (Number.isFinite(volumeRef.current) ? volumeRef.current : 0) + KEYBOARD_VOLUME_STEP));
      } else if (key === 'ArrowDown') {
        e.preventDefault();
        setVolume(Math.max(0, (Number.isFinite(volumeRef.current) ? volumeRef.current : 0) - KEYBOARD_VOLUME_STEP));
      } else if (key === 'm' || key === 'M') {
        e.preventDefault();
        toggleMute();
      }
  ...
  }, [togglePlay, playNext, playPrevious, seek, setVolume, toggleMute, audio]);
  ...
  useEffect(() => {
    if (!isAuthenticated) {
      hydratedRef.current = false;
      if (currentSongRef.current || queueRef.current.length) {
        clearQueue();
      }
    }
  }, [isAuthenticated, clearQueue]);

  // Dose 1.107: insertNext on idle player mirrors addToQueue — empty queue + no
  // currentSong → become current and autoplay; otherwise insert after current only.
