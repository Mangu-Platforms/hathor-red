...
export const PlayerProvider = ({ children }) => {
  ...
  const makeNext = useCallback((index) => {
    ...
    if (isShuffledRef.current && shuffleOrderRef.current.length === q.length) {
      ...
      let insertPos = shufflePosRef.current + 1;
      if (fromPos < insertPos) insertPos -= 1;
      insertPos = Math.max(0, Math.min(insertPos, order.length));
      order.splice(insertPos, 0, index);
      setShuffleOrder(order);
      return;
    }

    const qi = Number.isInteger(queueIndexRef.current) ? queueIndexRef.current : 0;
    const target = index < qi ? qi : qi + 1;
    if (target === index || target < 0 || target >= q.length) return;
    moveInQueue(index, target);
  }, [moveInQueue]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setShuffleOrder([]);
        setShufflePos(0);
        await loadSong(song, { autoplay: Boolean(state.isPlaying || state.playing), startAt });
        hydratedRef.current = true;
      } catch (err) {
  ...
  useEffect(() => {
    ...
    try {
      navigator.mediaSession.metadata = new MediaMetadata({
        ...
        album: song.album || '',
        artwork: song.coverUrl || song.cover_url
          ? [{ src: song.coverUrl || song.cover_url, sizes: '512x512', type: 'image/jpeg' }]
          : [],
      });
  ...
  useEffect(() => {
    ...
    const handlers = {
      ...
      seekbackward: (details) => {
        ...
        const off = (details && details.seekOffset) || MEDIA_SESSION_SEEK_SEC;
        seek(Math.max(0, t - off));
      },
      seekforward: (details) => {
        const t = Number.isFinite(audio.currentTime) ? audio.currentTime : 0;
        const d = Number.isFinite(audio.duration) && audio.duration > 0 ? audio.duration : t + MEDIA_SESSION_SEEK_SEC;
        const off = (details && details.seekOffset) || MEDIA_SESSION_SEEK_SEC;
        seek(Math.min(d, t + off));
      },
      seekto: (details) => {
