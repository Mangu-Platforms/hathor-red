    ...
    (async () => {
      try {
        ...
        if (state.volume != null && Number.isFinite(Number(state.volume))) {
          const v = Math.max(0, Math.min(1, Number(state.volume)));
          setVolumeState(v);
          if (v > 0) preMuteVolumeRef.current = v;
        }
        ...
        if (state.currentSongId && state.currentSong) {
          const song = state.currentSong;
          const startAt = Number.isFinite(state.position) ? Number(state.position) : 0;
          setQueue([song]);
          setQueueIndex(0);
