  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!state || state.current_song_id == null) {
          hydratedRef.current = true;
          return;
        }
        let song = null;
        try {
          song = await musicService.getSong(state.current_song_id);
        } catch (e) {
        ...
        if (Number.isFinite(vol) && vol >= 0 && vol <= 1) {
          setVolumeState(vol);
          if (vol > 0) preMuteVolumeRef.current = vol;
        }
        const spd = Number(state.playback_speed);
        ...
        console.warn('hydrate playback state failed', err);
      } finally {
