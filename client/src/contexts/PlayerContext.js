...
export const PlayerProvider = ({ children }) => {
  ...
  // Returns true when appended, false when skipped or invalid.
  ...
  // dose-1.98: insert song right after the current queue index ("Play next").
  // Updates shuffle order so the new track plays next in shuffle path too.
  // dose-1.99: skip if already in queue (same id).
  const insertNext = useCallback((song) => {
    ...
    setQueue((prev) => {
      ...
      const insertAt = Math.min(Math.max(0, queueIndexRef.current + 1), prev.length);
      const next = [...prev.slice(0, insertAt), song, ...prev.slice(insertAt)];
      setShuffleOrder((so) => {
        if (!so.length) return so;
        const remapped = so.map((i) => (i >= insertAt ? i + 1 : i));
        if (isShuffledRef.current) {
          const pos = shufflePosRef.current;
          const insertShuffleAt = Math.min(pos + 1, remapped.length);
          remapped.splice(insertShuffleAt, 0, insertAt);
        }
        return remapped;
      });
      return next;
  ...
  // dose-1.77: clearQueue must stop now-playing (same as emptying via remove),
  // record listen if ≥5s, and bump playGeneration so in-flight loadSong cannot resume.
  ...
  useEffect(() => {
    ...
    const onKey = (e) => {
      ...
      if (key === ' ' || key === 'k' || key === 'K') {
        ...
        seek(t0 + KEYBOARD_SEEK_SEC);
      } else if (key === 'ArrowLeft') {
        e.preventDefault();
        const t0 = Number.isFinite(audio.currentTime) ? audio.currentTime : 0;
        seek(Math.max(0, t0 - KEYBOARD_SEEK_SEC));
      } else if (key === 'ArrowUp') {
        e.preventDefault();
  ...
  }, [isAuthenticated, audio, togglePlay, seek, setVolume, toggleMute]);
  ...
  useEffect(() => {
    ...
    bind('play', () => play());
    bind('pause', () => pause());
    bind('previoustrack', () => playPrevious());
    bind('nexttrack', () => playNext());
    bind('seekbackward', () => {
      const t0 = Number.isFinite(audio.currentTime) ? audio.currentTime : 0;
      seek(Math.max(0, t0 - MEDIA_SESSION_SEEK_SEC));
    });
    bind('seekforward', () => {
      const t0 = Number.isFinite(audio.currentTime) ? audio.currentTime : 0;
      seek(t0 + MEDIA_SESSION_SEEK_SEC);
    });
    bind('seekto', (d) => {
  ...
  }, [audio, loadSong, playNext, playPrevious, persistPlayback, play, pause, seek]);
