...
export const PlayerProvider = ({ children }) => {
  ...
  // dose-1.74/1.75: record into listening_history when listen ≥ 5s (ended or skip/track-change).
  ...
  const playAtIndex = useCallback(
    async (index) => {
      ...
      // dose-1.75: record prior track when skipping / changing index if listened ≥ 5s
  ...
  const setQueueAndPlay = useCallback(
    async (songs, startIndex = 0) => {
      ...
      // dose-1.75: record prior track when a new queue replaces current song
      const prev = currentSongRef.current;
  ...
  const removeFromQueue = useCallback((index) => {
    setQueue((prev) => {
      ...
      // dose-1.75: record listen when removing now-playing after ≥ 5s
      if (removingCurrent) {
  ...
  }, [isAuthenticated, audio, togglePlay, seek, setVolume]);

  // dose-1.72 Media Session
  useEffect(() => {
