...
export const PlayerProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (state.repeatMode && ['none', 'all', 'one'].includes(state.repeatMode)) {
  ...
  }, [isAuthenticated, currentSong, isPlaying, volume, playbackSpeed, repeatMode, progress, audio]);

  // Flush position on tab hide / pagehide
  useEffect(() => {
