  ...
  useEffect(() => {
    ...
    const onError = async () => {
      ...
      try {
        ...
        console.warn('stream retry failed', err);
      }
    };
  ...
  useEffect(() => {
    ...
    try {
      ...
      navigator.mediaSession.setActionHandler('seekbackward', () => {
        const t = Number.isFinite(audio.currentTime) ? audio.currentTime : 0;
        seek(Math.max(0, t - MEDIA_SESSION_SEEK_SEC));
      });
      navigator.mediaSession.setActionHandler('seekforward', () => {
        const t = Number.isFinite(audio.currentTime) ? audio.currentTime : 0;
        const d = Number.isFinite(audio.duration) ? audio.duration : t + MEDIA_SESSION_SEEK_SEC;
        seek(Math.min(d, t + MEDIA_SESSION_SEEK_SEC));
      });
    } catch (_) {}
    return () => {
      try {
