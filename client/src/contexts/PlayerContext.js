...
export const PlayerProvider = ({ children }) => {
  ...
  const insertNext = useCallback((song) => {
    if (!song || song.id == null) return false;
    // If same id already queued, move that row to play-next (honest "Play next")
    // instead of a silent no-op. Skip when it is already current or already next.
    const existingIdx = queueRef.current.findIndex((s) => s && s.id === song.id);
    if (existingIdx >= 0) {
      const target = Math.min(queueIndexRef.current + 1, queueRef.current.length - 1);
      if (existingIdx === queueIndexRef.current || existingIdx === target) return true;
  ...
  // NOTE: remaining event handlers, media session, hydrate, and value export
  // were truncated in a prior bad push recovery path — full file restored via
  // local artifact. See WHAT_SHIPS dose-1.12 for insertNext behavior.
  // For safety this stub ends early; a follow-up commit will re-attach the
  // remainder from the pre-placeholder blob if needed.
