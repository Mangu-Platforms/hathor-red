  ...
  io.on('connection', (socket) => {
    ...
    socket.on('sync-state', async (raw) => {
      try {
        ...
        if (Object.prototype.hasOwnProperty.call(state, 'currentSongId') && state.currentSongId != null) {
          const sid = toPositiveInt(state.currentSongId);
          if (sid == null) return;
          currentSongId = sid;
        }

        const toBound = (v, min, max) => {
          if (v == null) return null;
          const n = Number(v);
          if (!Number.isFinite(n) || n < min || n > max) return null;
          return n;
        };
        const position = toBound(state.position, 0, 7200) ?? 0;
        const volume = toBound(state.volume, 0, 1) ?? 1;
        const playbackSpeed = toBound(state.playbackSpeed, 0.25, 4) ?? 1;
        const isPlaying = typeof state.isPlaying === 'boolean' ? state.isPlaying : false;

        let pitchShift = null;
        if (Object.prototype.hasOwnProperty.call(state, 'pitchShift') && state.pitchShift != null) {
          const n = Number(state.pitchShift);
          if (!Number.isFinite(n) || n < -24 || n > 24) return;
          pitchShift = n;
        }
        let stemsConfig = null;
        if (Object.prototype.hasOwnProperty.call(state, 'stemsConfig') && state.stemsConfig != null) {
          if (typeof state.stemsConfig !== 'object' || Array.isArray(state.stemsConfig)) return;
          stemsConfig = state.stemsConfig;
        }
