...
function getRoomPresenceRoster(roomId) {
  // dose-1.41: same positive-int bar as room-control / socket auth
  const id = toPositiveInt(roomId);
  if (id == null) return [];
  return presenceRoster(id);
}

// Lightweight per-user event throttle: `max` events per rolling second.
...
const setupSocketHandlers = (io) => {
  io.use((socket, next) => {
    ...
    try {
      ...
      // dose-1.39: same bar as authMiddleware / streamAuth — reject stream-typed
      // tokens and require finite positive integer userId before socket.userId is set.
      if (decoded.typ === 'stream') {
        return next(new Error('Stream tokens cannot authenticate sockets'));
      }
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('join-room', async (roomId) => {
      try {
        // dose-1.41: shared toPositiveInt (same bar as room-control)
    ...
    socket.on('leave-room', async (roomId) => {
      try {
        // dose-1.41: shared toPositiveInt
        const roomIdNum = toPositiveInt(roomId);
    ...
    const relayRtc = (eventName) => (data) => {
      try {
        ...
        // dose-1.41: shared toPositiveInt for room + target
        const roomIdNum = toPositiveInt(roomId);
    ...
    // dose-1.40: room-control uses shared toPositiveInt for roomId/songId and
    // bounds position to finite [0, 7200] (same bar as sync-state / HTTP playback)
    // so a hostile host client cannot write NaN/junk into listening_rooms.
    ...
    // dose-1.39: same defense-in-depth bar as HTTP updatePlaybackState so a
    // hostile/broken client cannot write NaN/junk into playback_states via socket.
    socket.on('sync-state', async (raw) => {
      try {
        ...
        if (Object.prototype.hasOwnProperty.call(state, 'currentSongId')) {
          if (state.currentSongId === null) {
            ...
            const sid = toPositiveInt(state.currentSongId);
            if (sid == null) return; // drop invalid
            currentSongId = sid;
          }
        } else {
          // key absent — leave prior; for simplicity require explicit or skip write
          return;
        }

        let position = 0;
        if (Object.prototype.hasOwnProperty.call(state, 'position')) {
          const n = Number(state.position);
        ...
        if (Object.prototype.hasOwnProperty.call(state, 'isPlaying')) {
          if (typeof state.isPlaying !== 'boolean') return;
          isPlaying = state.isPlaying;
        }

        let volume = 1;
        if (Object.prototype.hasOwnProperty.call(state, 'volume')) {
          const n = Number(state.volume);
        ...
        if (Object.prototype.hasOwnProperty.call(state, 'playbackSpeed')) {
          const n = Number(state.playbackSpeed);
          if (!Number.isFinite(n) || n < 0.25 || n > 4) return;
          playbackSpeed = n;
        }

        const result = await db.query(
          `INSERT INTO playback_states (user_id, current_song_id, position, is_playing, volume, playback_speed, updated_at)
           ...
           RETURNING *`,
          [socket.userId, currentSongId, position, isPlaying, volume, playbackSpeed]
        );
        ...
        try {
          if (redisClient && redisClient.isReady) {
            await redisClient.setEx(
              `playback:${socket.userId}`,
              3600,
              JSON.stringify(row)
            );
          }
        } catch (redisErr) {
          logger.warn(`Redis playback cache write failed: ${redisErr.message}`);
        }
      } catch (error) {
