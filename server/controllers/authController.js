...
const getListeningStats = async (req, res) => {
  try {
    ...
    // dose-1.64: stats counts use shared toNonNegInt (finite non-negative integer;
    // allow 0; reject NaN/negative/non-integer instead of raw parseInt).
    res.json({
      totalPlays: toNonNegInt(totalPlays.rows[0].count) ?? 0,
      totalListeningTimeSeconds: toNonNegInt(totalTime.rows[0].total_seconds) ?? 0,
